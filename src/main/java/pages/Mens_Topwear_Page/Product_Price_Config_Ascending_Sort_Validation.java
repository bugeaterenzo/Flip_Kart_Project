...
public class Product_Price_Config_Ascending_Sort_Validation {
    ...
    public Product_Price_Config_Ascending_Sort_Validation(WebDriver driver)
    {
        this.driver = driver;
    ...
    public void navigate_to_mens_top_wear_page()
    {

        WebElement clickable_fashion_btn = Wait_Utils.wait_for_element(driver, fashion_btn, WaitStrategy.CLICKABLE);
        action.moveToElement(clickable_fashion_btn).perform();

        WebElement clickable_mens_top_wear_btn = Wait_Utils.wait_for_element(driver, mens_top_wear_btn, WaitStrategy.CLICKABLE);
        action.moveToElement(clickable_mens_top_wear_btn).click().perform();
    ...
    {
        WebElement clickable_Price_Low_to_High = Wait_Utils.wait_for_element(driver, Price_Low_to_High, WaitStrategy.CLICKABLE);
        Wait_Utils.global_thread_pause(4);
        action.moveToElement(clickable_Price_Low_to_High).click().perform();
        Wait_Utils.global_thread_pause(10);
    }

    public boolean validate_ascending_price_list()
    {
...
//                        ":not(:has(div._0BvurA))"));
        ...
        for (int i = 0; i < prices.size() - 1; i++)
        {
            ...
            {
                System.out.println("Price list is not sorted correctly! Issue found at index: " + i);
                result = false;
            }
        }
        ...
        if (result)
        {
            System.out.println("\nPrice list is sorted in ascending order.");
        }
